...
%
classdef SymphonyV2ParserJ < parsers.SymphonyParserJHDF5
    ...
    methods
        ...
        function obj = SymphonyV2ParserJ(fname)
        ...
        function obj = parse(obj)
            ...
            sourceLinks = obj.reader.getGroupMemberInformation(m.getPath(), true); % Collect addresses of all sourceLinks            
            m = sourceLinks.get(1);
            epochGroups = obj.reader.getGroupMemberInformation(m.getPath(), true); % Collect addresses of all EpochGroups 
            epochsByCellMap = obj.getEpochsByCellLabel(epochGroups);            
            %m.getPath()
            %epochsByCellMap = obj.getEpochsByCellLabel(obj.info.Groups(1).Groups(2).Groups);

            m = rootMembers.get(5); % Get the experiment root
            sourceLinks = obj.reader.getGroupMemberInformation(m.getPath(), true); % Go to the next level down            
...
%             end
        ...
        function cell = buildCellData(obj, label, h5Epochs)
            ...
            for i = 1 : numel(h5Epochs)
                ...
                if ~ strcmp(protocolId, lastProtocolId)
                    ...
                    % add epoch group properties to current prtoocol parameters
        ...
        function epochGroupMap = getEpochsByCellLabel(obj, epochGroups)
            ...
            for i = 0 : epochGroups.size()-1
                m = epochGroups.get(i);
                h5Epochs = obj.reader.getGroupMemberInformation(m.getPath(), true);
                %h5Epochs = flattenByProtocol(epochGroups(i).Groups(1).Groups);
                label = obj.getSourceLabel(epochGroups.get(i));
        ...
        function label = getSourceLabel(obj, epochGroup)
            ...
            % if not present it should be in links (TODO: removed, need to check this)            
            groups = obj.reader.getGroupMemberInformation(epochGroup.getPath(), true);
            if groups.size() >= 4
                m       = groups.get(groups.size()-1);
                obj.parseH5Name(char(m.getPath()));
                atts    = obj.mapAttributes(m.getPath());
                label   = atts('label');
            end
        end
